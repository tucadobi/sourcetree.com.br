...
int read_file(int fd, struct region *region)
{
    ...
    /**
     * "len+8" garante que haja algum desleixo no final.
     */
...
static int parse_pes_stitches(struct region *region, unsigned int pec, struct pes *pes)
{
    ...
    while (p < end)
    {
        ...
        /**
         * Conjunto de bits alto significa deslocamento
         * de 12 bits, caso contrário, delta assinado
         * de 7 bits.
         */
        if (val1 & 0x80)
        {
            ...
            /**
             * Aritmética assinada de 12 bits.
             */
            if (val1 & 2048)
            {
                val1 -= 4096;
