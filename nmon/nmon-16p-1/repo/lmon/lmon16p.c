/**
 * lmon.c - Monitor de desempenho baseado em Curses para Linux com
 *          salvamento de estatísticas de desempenho em um modo de
 *          arquivo CSV.
 *
...
/**
 * Use o Makefile a seguir (para Linux no POWER).
 *
...
/**
 * Outros #ifdef para recursos ou plataformas específicas.
 * 
 * Platforms: POWER MAINFRAME X86 ARM - Obrigatório um destes de cada vez.
 * Specific Linux versions: RHEL7 SLES11 SLES12.
 * Specific feature: NVIDIA_GPU.
 * 
 * Bug / soluções alternativas de recursos ausentes:
 *     REREAD - para RHEL3.
 *     LSBLK_NO_TYPE - SLES11 não tem opção lsblk disk TYPE.
 *
 * Opções que devem estar sempre ativadas:
 * SMALLMEM - Remove memória enorme, suja, whritebak, mapeada,
 *            laje, threads de página, como não em kernels
 *            mais antigos.
 *
 * PRE_KERNEL_2_6_18 - 1 níveis de kernel antes de remover o seguinte
 *                     para as estatísticas do disco.
 *     pi_num_threads,
 *     pi_rt_priority,
 *     pi_policy,
...
/**
 * Observação: RAW assume que você está usando o índice “i”
 *             para selecionar a CPU.
 */
...
/**
 * Para saída estilo Disk Busy Rain cobrindo centenas
...
/**
 * Reportar dados adicionais de /proc/diskstats para
 * a saída da planilha.
 */
int extended_disk = 0;
...
/**
 * Corte de tudo após o primeiro espaço no retorno de chamada.
 * Exclua qualquer '&' imediatamente antes do espaço.
 */
...
/**
 * Remova a saída de erro deste buffer e exiba-a se NMONDEBUG=1.
 */
char errorstr[70];
    ...
    if (pop != NULL)
    {
        ...
        while (fgets(tmpstr, LSCPU_STRLEN, pop) != NULL)
        {
            ...
            if (strncmp("Architecture:", tmpstr, strlen("Architecture:")) == 0)
            {
                ...
                /**
                 * Comece com char após :<space> procurando por algumas
                 * letras ou números.
                 */
                len = strlen(tmpstr);
...
/**
 * Número máximo de linhas em arquivos /proc. A Intel já tem 26 (então
 * aqui 30) por CPU Hypterthread (máximo de 128 * 2 CPUs aqui). POWER
 * tem apenas 6 a 7 linhas, mas logo obtém 1536 threads SMT. Errando
 * pelo lado seguro abaixo. MAGIC COOKIE WARNING.
 */
#define PROC_MAXLINES (16 * 1024)

/**
 * Sinalize se já executamos a função proc_cpu()
 * neste intervalo.
 */
int proc_cpu_done = 0;
...
struct {
    ...
    /**
     * Rastreie atualizações para cada atualização para impedir
     * a coleta dupla de dados.
     */
...
void proc_read(int num)
{
    ...
    /**
     * Defina o sinalizador para não relermos os dados, mesmo se
     * chamados várias vezes no mesmo intervalo.
     */
...
struct procsinfo {
    ...
    /**
     * A informação de signal é obsoleto.
     */
...
/**
 * Número de CPUs no sistema (esperemos que seja mais
 * que zero !).
 */
int cpus = 1;
...
/**
 * Forma abreviada de dados do arquivo de partições (ou seja,
 * dados ausentes).
 */
 ...
 * que não ultrapasse o máximo da linha de entrada da
 * planilha.
 */
#define DPL 150
...
/**
 * 1=all procs& disk 0=only if 1% or more busy.
 */
int show_all = 1;
 ...
 * 0 = usando curses em preto e branco (veja a flag -b).
 */
int colour = 1;
...
/**
 * 1 = exibir as caixas na tela
 * 0 = sem caixas, mas "---" entre diferentes estatísticas
 *     de medição, consulte a opção -B.
 */
int show_box = 1;
 ...
 */
    ...
    /**
     * XXXXXXX preciso testar se rewind() funcionou ou não
     * para lparcfg.
     */
     ...
     * 
     * Observação: lparcfg não é lido via proc_read() !
     */
    ...
    struct {
        ...
        /**
         * Novo registro é "IBM pSeries (emulado por qemu)"
    ...
    int proc_lparcfg()
    {
        ...
        if (lparcfg_reread)
        {
             ...
             * necessário - infelizmente, isso requer a instalação de
             * muitas versões do Linux no POWER para testar os primeiros
             * lançamentos.
             */
            fclose(fp);
        ...
        /**
         * Remova novos espaços em uma string enorme,
         * significando PowerKVM Guest !!
         */
        ...
        {
            /**
             * Heurísticas para detectar PowerKVM Guest.
             *     a) dentro de ifdef POWER, então não pode ser x86.
             *     b) temos um /proc/ppc64/lparcfg - provavelmente
             *        quase ausente (1.9).
             *
             *     c) string de tipo de sistema inclui "qemu".
             *        Confirmado como verdadeiro para SLES11 RHEL6.5 e
             *        Ubuntu 14.4.1.
             */
        ...
        /**
         * A força bruta pode fornecer dados temporários incorretos
        ...
        if (lparcfg.cmo_enabled)
        {
            ...
            /**
             * Tamanho total do pool do AMS em bytes.
             */
            GETDATA(entitled_memory_pool_size);

            /**
             * AMS solicita mais empréstimo de memória.
             */
...
/**
 * Suporta até 780, mas não acompanha POWER6 595
 * com POWER7.
 *
 * XXXX - precisa de retrabalho para lidar com um
 * número bastante raro, mas interessante, de
 * máquinas com CPU.
 *
 * MAGIC COOKIE WARNING.
 */
...
/**
 * ptr para array  - 1 para loop da cpu - 0 = média para máquina.
 */
double *cpu_peak;
...
/**
 * Converta secs + micro secs em duplo.
 */
...
void get_cpu_cnt()
{
    ...
    /**
     * Obtenha informações da CPU em /proc/stat e
     * preencha proc[P_STAT].
     */
    proc_read(P_STAT);
...
void proc_cpu()
{
    ...
    {
        stat8 = sscanf(&proc[P_STAT].line[0][5], "%lld %lld %lld %lld %lld %lld %lld %lld", &user, &nice, &sys, &idle, &iowait, &hardirq, &softirq, &steal);
        stat10 = sscanf(&proc[P_STAT].line[0][5], "%lld %lld %lld %lld %lld %lld %lld %lld %lld %lld", &user, &nice, &sys, &idle, &iowait, &hardirq, &softirq, &steal, &guest, &guest_nice);
    ...
    if (stat10 == 10)
    {
        sscanf(&proc[P_STAT].line[0][5], "%lld %lld %lld %lld %lld %lld %lld %lld %lld %lld", &user, &nice, &sys, &idle, &iowait, &hardirq, &softirq, &steal, &guest, &guest_nice);
    } else
    {
        if (stat8 == 8)
        {
            sscanf(&proc[P_STAT].line[0][5], "%lld %lld %lld %lld %lld %lld %lld %lld", &user, &nice, &sys, &idle, &iowait, &hardirq, &softirq, &steal);
        } else
        {
            /**
             * stat 4 variáveis aqui como proc Linux mais antigo.
             */
            sscanf(&proc[P_STAT].line[0][5], "%lld %lld %lld %lld", &user, &nice, &sys, &idle);
        }
    }
    ...
    p->cpu_total.guest_nice = guest_nice;
        ...
        if (debug)
        {
            fprintf(stderr, "XX user=%lld wait=%lld sys=%lld idle=%lld\n", p->cpu_total.user, p->cpu_total.wait, p->cpu_total.sys, p->cpu_total.idle);
        }
    #endif
...
void proc_nfs()
{
    ...
    if (proc[P_NFS].fp != 0)
    {
        for (lineno = 0; lineno < proc[P_NFS].lines; lineno++)
        {
            ...
            {
                /**
                 * Leitores de linha da versão 2 do cliente "proc2 18 num num etc".
                 */
    ...
    if (proc[P_NFSD].fp != 0)
    {
        for (lineno = 0; lineno < proc[P_NFSD].lines; lineno++)
        {
            ...
            {
                /**
                 * Leitores de linha da versão 2 do servidor "proc2 18 num num etc".
                 */
            ...
            {
                /**
                 * Leitores de linha versão 4 do servidor "proc4ops 40 num num etc".
                 * Observação: as "ops", portanto, começando na coluna 9.
                 */
...
void proc_diskstats(double elapsed)
{
    ...
    for (i = 0; i < DISKMAX;)
    {
        ...
        ret =
            sscanf(&buf[0], "%d %d %s %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu %lu",
                &p->dk[i].dk_major,
                &p->dk[i].dk_minor,
                &p->dk[i].dk_name[0],
                &p->dk[i].dk_reads,
...
void proc_partitions(double elapsed)
{
    ...
    for (i = 0; i < DISKMAX; i++)
    {
        ...
        /**
         * Em milli-seconds para fazer isso upto 100%, 1000/100 = 10.
         */
        p->dk[i].dk_time /= 10.0;
...
void plot_smp(WINDOW * pad, int cpu_no, int row, double user, double kernel, double iowait, double idle, double steal)
{
    ...
    if (cursed)
    {
        ...
        {
            /**
             * adicionado "=" para tentar neutralizar as metades faltantes.
             */
            #ifdef POWER
...
void interrupt(int signum)
{
    ...
    {
        /**
         * Curses stop + start, então calcula o # número de linhas e colunas.
         */
        CURSE endwin();
    ...
    for (dgroup_total_groups = 0; fgets(line, 4096 - 1, gp) != NULL && dgroup_total_groups < DGROUPS; dgroup_total_groups++)
    {
        ...
        for (i = 0; i < DGROUPITEMS && *nextp != 0; i++)
        {
            ...
            for (j = 0; j < disks; j++)
            {
                ...
                {
                    /**
                     * DEBUG printf("DGadd group=%s,name=%s,disk=%s,dgroup_total_groups=%d,dgroup_total_disks=%d,j=%d,i=%d,index=%d.\n", dgroup_name[dgroup_total_groups], name, dk[j].dk_name, dgroup_total_groups, dgroup_total_disks, j, i,dgroup_total_groups*DGROUPITEMS+i);
                     */
...
/**
 * Ordenamos este array em vez das tabelas de processos reais
 * o índice é a posição na tabela de processos e o tamanho é
 * a memória usada em bytes o io é a E/S de armazenamento
 * executada no último período em bytes o tempo é a CPU
 * usada no último período em segundos.
 */
struct topper {
    /**
     *
...
/**
 * Forçar a exibição de estatísticas do disco em
 * KB, MB, GB.
 */
int minimum_units = 'K';
                ...
                switch (buf[i])
                {
                    ...
                    case '.':
                        ...
                        } else
                        {
                            ...
                            /**
                             * Mudar para a visualização favorita confunde os
                             * outros, então desative esse recurso.
                             *
...
int getprocs(int records)
{
    ...
    while ((char *) (dent = readdir(procdir)) != NULL)
    {
        if (dent->d_type == 4)
        {
            /**
             * Este é um diretório.
             * mainframes report 0 = desconhecido toda vez.
             */
...
/**
 * Inicie o processo conforme especificado em cmd em um
 * processo filho sem aguardar a conclusão. não tenho
 * certeza se deseja evitar esta funcionalidade para
 * o usuário root.
 * 
...
void child_start(int when, char *cmd, int timestamp_type, int loop, time_t the_time)
{
    ...
    /**
     * Usado para resolver hour/min/second.
     */
    struct tm *tim;
    ...
    switch (child_pid = fork())
    {
        ...
        case 0:
            ...
            /**
             * Feche todos os ponteiros de arquivo abertos,
             * exceto os padrões.
             */
            for (i = 3; i < 5; ++i)
            {
            ...
            /**
             * Se chegarmos aqui, o comando especificado não
             * poderá ser iniciado.
             */
            perror(when_info);
...
int main(int argc, char **argv)
{
    ...
    /**
     * Sinalize que tipo de carimbo de data/hora damos
     * às crianças iniciadas.
     *     0: "T%04d".
    ...
    /**
     * Relógio ticks por segundo usado em linhas de
     * CPU /proc/stat.
     */
    long ticks = 100;

    /**
     * O tamanho de página padrão é 4KB, mas os servidores
    ...
    /**
     * Obtenha os tiques do relógio por segundo para
    ...
    while (-1 != (i = getopt(argc, argv, "?abBc:C:Dd:EfF:g:hI:Jl:m:MNpr:Rs:tTUVxXz^")))
    {
        switch (i)
        {
            case '?':
            ...
            case 'g':
                ...
                if (strncmp("auto", dgroup_filename, 5) == 0)
                {
                    ...
                    #ifdef SLES11
                        /**
                         * Defina isso para contornar o recurso --output
                         * TYPE ausente.
                         */
            ...
            case 't':
                /**
                 * Coloque a saída principal do processo no modo de
                 * planilha.
                 */
                show_top = 1;
                show_topmode = 3;
    ...
    /**
     * Defina os parâmetros se não forem definidos acima.
     */
    if (maxloops == -1)
    {
        maxloops = 9999999;
     ...
     * todos os outros conjuntos de dados também para não
     * obtermos dados incorretos para o primeiro intervalo
     * relatado.
     */
    if (!cursed)
    {
        /**
         * Obter estatísticas VM.
         */
        read_vmstat();
    ...
    } else
    {
        ...
        if (varperftmp)
        {
            ...
            /**
             * Nome do host mais tamanho do diretório mais o
             * número.
             */
        ...
        {
            open_filename = MALLOC(strlen(hostname) + 64);
            snprintf(open_filename, strlen(hostname) + 63,
                "%s_%02d%02d%02d_%02d%02d.nmon",
                hostname,
                tim->tm_year,
                tim->tm_mon,
                tim->tm_mday,
        ...
        /**
         * iremoved pois não está abaixo da linha BUSY fprintf(fp,"\n");
         */
        #ifdef DEBUG
            if (debug)
            {
        ...
        if(show_jfs)
        {
            ...
            for (k = 0; k < jfses; k++)
            {
                if (jfs[k].mounted && strncmp(jfs[k].name, "/proc", 5) && strncmp(jfs[k].name, "/sys", 4) && strncmp(jfs[k].name, "/run/", 5) && strncmp(jfs[k].name, "/dev/", 5) && strncmp(jfs[k].name, "/var/lib/nfs/rpc", 16))
                {
                    /**
        ...
        /**
         * Para obter as primeiras estatísticas para cobrir
         * esse segundo e evitar problemas de divisão por
         * zero.
         */
        sleep(1);
    }
    ...
    for (loop = 1;; loop++)
    {
        ...
        get_cpu_cnt();
             ...
             * dados mais significativos. O valor de retorno é
             * ignorado aqui, mas lembrado em proc_lparcfg().
             */
            proc_lparcfg();
        #endif
        ...
        if (show_res && cursed)
        {
            ...
            COLOUR wattrset(padres, COLOR_PAIR(3));
                ...
                mvwprintw(padres, 8, 4, "cpuinfo: %s    %s", proc[P_CPUINFO].line[proc[P_CPUINFO].lines - 2], proc[P_CPUINFO].line[proc[P_CPUINFO].lines - 1]);
                ...
                {
                    case VM_POWERKVM_GUEST:
                        mvwprintw(padres, 9, 20, "PowerKVM Guest Physical CPU:%d & Virtual CPU (SMT):%d  %s", lparcfg.partition_active_processors, cpus, lscpu.byte_order);
                        break;
                    ...
                    case VM_POWERVM:
                        mvwprintw(padres, 9, 20, "PowerVM Physical CPU:%d & Logical CPU:%d  %s", lparcfg.partition_active_processors, cpus, lscpu.byte_order);
                        break;
        ...
        if (show_longterm)
        {
             ...
             * Estes são um acréscimo aos 100%.
             */
        ...
        if (show_smp || show_verbose || show_wide)
        {
            ...
            if (old_cpus != cpus)
            {
                ...
                } else
                {
                     ...
                     * Faça a substituição da força bruta dos dados anteriores.
                     */
                    if (cpus < old_cpus)
                    {
            ...
            if (show_smp)
            {
                ...
                /**
                 * if (show_smp) [e] if (cursed).
                 */
                #ifdef POWER
                    /**
                     * Sempre obtenha informações lpar também para que possamos
                     * relatar o uso físico da CPU e tornar os dados mais
                     * significativos. Isso pressupõe que as informações da
                     * LPAR estejam disponíveis em q e p !
                     */
                    if (proc_lparcfg() > 0)
                    {
                        if (lparcfg.shared_processor_mode == 1)
                        {
                            ...
                            {
                                /**
                                 * Host, convidado ou nativo do PowerKVM não possuem
                ...
                /**
                 * + RAWTOTAL(guest) + RAWTOTAL(guest_nice); estes
                 * são um acréscimo aos 100%.
                 */
                cpu_wait = RAWTOTAL(wait);
                cpu_idle = RAWTOTAL(idle);
                cpu_steal = RAWTOTAL(steal);
                ...
                if (cursed)
                {
                    ...
                    } else
                    {
                        for (i = 0; i < cpus && i < 64; i++)
                        {
                            ...
                            /**
                             * + RAW(guest) + RAW(guest_nice); estes são
                             * adicionais ao 100%.
                             */
        ...
        if (show_kernel)
        {
            ...
            } else
            {
                 ...
                 * (float) p->cpu_total.blocked,                                ~> 2 swapin (# de processos aguardando a conclusão do IO.
            ...
            if (cursed)
            {
                ...
                if (nfs_v3c_found || nfs_v3s_found)
                {
                    for (i = 0; i < 18; i++)
                    {
                        ...
                        if (show_nfs == 1)
                        {
                            mvwprintw(padnfs, 2 + i, 3, "%12s %8.1f %8.1f", nfs_v2_names[i], NFS_DELTA(nfs.v2c[i]), NFS_DELTA(nfs.v2s[i]));
                        }
                ...
                if (nfs_v3c_found || nfs_v3s_found)
                {
                    for (i = 0; i < 22; i++)
                    {
                        ...
                        if (show_nfs == 1)
                        {
                            mvwprintw(padnfs, 2 + i, 41, "%12s %8.1f %8.1f", nfs_v3_names[i], NFS_DELTA(nfs.v3c[i]), NFS_DELTA(nfs.v3s[i]));
                        }
        ...
        for (i = 0; i < networks; i++)
        {
            errors += p->ifnets[i].if_ierrs -
                      q->ifnets[i].if_ierrs +
                      p->ifnets[i].if_oerrs -
                      q->ifnets[i].if_oerrs +
                      p->ifnets[i].if_ocolls -
                      q->ifnets[i].if_ocolls;
        }
        ...
        if (show_jfs)
        {
            if (cursed)
            {
                ...
                for (k =0, j = 0; k < JFSMAX && j < jfses; k++)
                {
                    ...
                    /**
                     * /proc/ ~> subdiretórios também devem ser falsos.
                     * /proc  ~> um a mais que a string para garantir o NULL.
                     */
                    ...
                    {
                        /**
                         * /proc dá valores inválido/insano.
                         */
                        if (show_jfs_minimum)
                        {
                            /**
        ...
        if (show_disk)
        {
            if (cursed)
            {
                if (show_disk)
                {
                    ...
                    mvwprintw(paddisk, 0, 47, "Warning:may contains duplicates");
                    ...
                    switch (show_disk)
                    {
                        ...
                        case SHOW_DISK_GRAPH:
                             ...
                             */
                            COLOUR wattrset(paddisk, COLOR_PAIR(0));
                            mvwprintw(paddisk, 1, 27, "|0          |25         |50          |75       100|");
                            break;
                    }
                }
                ...
                } else
                {
                    ...
                    if (show_disk == SHOW_DISK_GRAPH)
                    {
                        for (i = 0, k = 0; i < disks; i++)
                        {
                            ...
                            /**
                             * Tamanho máximo de exibição.
                             */
                            if (disk_read  > 99999.9 || disk_write > 99999.9)
                            {
                                if (minimum_units == 'K')
                                {
                                    minimum_units = 'M';
                            ...
                            if (disk_read  > 99999.9 * 1024.0 || disk_write > 99999.9 * 1024.0)
                            {
                    ...
                    for (i = 0, k = 0; i < disks; i++)
                    {
                        ...
                        if (show_disk == SHOW_DISK_STATS)
                        {
                            ...
                            mvwprintw(paddisk, 2 + k, 13, "%9.1f", disk_read / minimum_units_factor() );
                            COLOUR wattrset(paddisk, COLOR_PAIR(3));

                            mvwprintw(paddisk, 2 + k, 22, "%9.1f", disk_write / minimum_units_factor() );
                        ...
                        if (show_disk == SHOW_DISK_GRAPH)
                        {
                            ...
                            {
                                /**
                                 * 50 colunas na área do gráfico do disco,
                                 * então divida % por dois.
                                 */
                                ...
                                /**
                                 * Não vá além da linha 78, ou seja,
                                 * j = 28 + 50.
                                 */
        ...
        } else
        {
            ...
            {
                /**
                 * Reduzir o tempo de sono em um número inteiro
                 * de segundos.
                 */
